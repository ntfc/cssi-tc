...
public class Frequencies {
  ...
  // calculate text entropy: http://stackoverflow.com/a/9612275/1975046
  // see also: http://cs.stackexchange.com/a/449
  public static double calcEntropy(String c) {
    int[] f = lettersFrequency(c);
    int len = c.length();
    double sum = 0.0;
    for(int i = 0; i < f.length; i++) {
      double fi = (double)f[i] / (double)len;
      sum += fi * (Math.log(fi) / Math.log(2));
    }
    return -sum;
  }
